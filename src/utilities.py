...
class Utilities:
    ...
    def get_tiles_location():
        if Utilities.tiles_location_instance is None:
            storage_provider = config['storage_provider'].upper()

            if (storage_provider == StorageProvider.FS):
                Utilities.tiles_location_instance = config["fs"]["internal_outputs_path"]

            elif (storage_provider == StorageProvider.S3):
                bucket = config["s3"]["bucket"]
                s3_path = '/vsis3/{0}'.format(bucket)
                Utilities.tiles_location_instance = s3_path

        return Utilities.tiles_location_instance
